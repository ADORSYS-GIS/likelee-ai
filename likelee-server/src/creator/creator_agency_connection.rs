...
pub struct CreatorAgencyInvite {
    ...
    pub marketplace_contract:
        Option<crate::agency::agency_marketplace_contracts::MarketplaceContractSummary>,
}
...
pub async fn list_invites(
    ...
    crate::agency::agency_marketplace_contracts::sync_open_contracts_for_creator(&state, &creator_id)
        .await?;
    ...
    for invite in &mut invites {
        invite.marketplace_contract =
            crate::agency::agency_marketplace_contracts::get_latest_contract_for_pair(
                &state,
                &invite.agency_id,
                &invite.creator_id,
            )
...
pub async fn accept_invite(
    ...
    if let Some(contract_row) = contract_rows.first() {
        let synced =
            crate::agency::agency_marketplace_contracts::sync_contract_for_row(&state, contract_row)
                .await?;
...
pub struct ContractSummaryResponse {
    pub status: String,
    pub contract: Option<crate::agency::agency_marketplace_contracts::MarketplaceContractSummary>,
}
...
pub async fn list_connections(
    ...
    for connection in &mut connections {
        connection.marketplace_contract =
            crate::agency::agency_marketplace_contracts::get_latest_live_contract_for_pair(
                &state,
...
async fn get_latest_contract_for_connection(
    ...
    crate::agency::agency_marketplace_contracts::get_latest_contract_row_for_pair(
...
async fn notify_agency_about_disconnect_request(
    ...
    let Ok((agency_name, agency_email)) =
        crate::agency::agency_marketplace_contracts::resolve_agency_identity(state, agency_id).await
    else {
        return;
    };
...
pub async fn disconnect_agency(
        ...
        get_latest_contract_for_connection(&state, &agency_id, &creator_id).await?
    {
        ...
        if contract_status == "expired" || contract_status == "terminated" {
            crate::agency::agency_marketplace_contracts::remove_live_connection_for_contract_row(
                &state,
                &contract_row,
            )
            .await;
...
pub async fn approve_disconnect_request(
    ...
    crate::agency::agency_marketplace_contracts::sync_open_contracts_for_agency(&state, agency_id).await?;
...
pub async fn get_agency_contract_summary(
    ...
    RoleGuard::new(vec!["agency"]).check(&user.role)?;
    crate::agency::agency_marketplace_contracts::sync_open_contracts_for_agency(&state, &user.id).await?;
    let contract = crate::agency::agency_marketplace_contracts::get_latest_live_contract_for_pair(
        &state,
        &user.id,
        &creator_id,
    )
