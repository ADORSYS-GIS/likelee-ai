use crate::admin::activity::log_activity_event_with_subject;
use crate::brand::brand_campaigns::{resolve_agency_name, resolve_brand_name, resolve_creator_name};
use crate::config::AppState;
use crate::billing::entitlements::{brand_allows_campaign_collaboration, get_brand_plan_tier};

use crate::errors::sanitize_db_error;
use crate::billing::pricing_defaults::{is_default_pricing, should_default_visibility_on};
...
pub async fn search_marketplace_profiles(
    ...
    if entity_type == "creator" {
        ...
        if user.role == "agency" {
            crate::agency::agency_marketplace_contracts::sync_open_contracts_for_agency(
                &state,
                &effective_agency_id,
            )
            .await?;
        ...
        for row in creators_rows {
            ...
            let marketplace_contract = contract_by_creator_id
                .get(creator_id)
                .map(crate::agency::agency_marketplace_contracts::parse_contract_summary)
...
pub async fn create_marketplace_connection_request(
    ...
    if profile_type == "creator" {
        ...
        crate::billing::entitlements::require_agency_paid_access(
            &state,
            &effective_agency_id,
        ...
        let contract_response =
            crate::agency::agency_marketplace_contracts::create_marketplace_connect_contract(
                &state,
                &effective_agency_id,
                &creator_id,
                &payload,
