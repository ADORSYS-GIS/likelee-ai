...
pub async fn create_campaign_offer_checkout(
    ...
    if target_type == "agency" && billing_request_id.is_empty() {
        match crate::brand::brand_campaigns::ensure_campaign_billing_stub(&state, &offer_id).await {
            Ok(stub_id) => {
...
pub async fn check_budget_alerts_cron(
    ...
    for brand in brands {
        ...
        if spend_percent >= 100.0 && !alert_100_sent {
            if let Err((code, err)) = crate::messaging::notifications::send_brand_notification(
                &state,
                brand_id,
                None,
...
pub async fn check_license_expiration_alerts_cron(
    ...
    match crate::licensing::licensing_requests::check_license_expiration_alerts_all_brands(&state).await {
...
pub async fn auto_archive_expired_licensing_requests_cron(
    ...
    match crate::licensing::licensing_requests::auto_archive_expired_licensing_requests(&state).await {
