...
async fn check_customer_default_payment_method(
    state: &AppState,
    customer_id: &str,
) -> Result<bool, String> {
    if state.stripe.secret_key.trim().is_empty() {
        return Ok(false);
    }

    let client = stripe_sdk::Client::new(state.stripe.secret_key.clone());
    ...
    match stripe_sdk::Customer::retrieve(&client, &cust_id, &[]).await {
        Ok(customer) => {
            let has_default = customer
                .invoice_settings
                .as_ref()
                .and_then(|is| is.default_payment_method.as_ref())
                .is_some();
            Ok(has_default)
        }
        Err(e) => {
            warn!(error = %e, customer_id = %customer_id, "failed to retrieve customer for default payment method check");
            Ok(false)
        }
    }
}
...
pub async fn get_agency_seat_breakdown(
    ...
    for sub in subscriptions.iter() {
        ...
        if !crate::billing::payouts::stripe_subscription_is_active(sub) {
            continue;
        }
...
pub async fn create_checkout_session_legacy(
    ...
    if state.stripe.studio_success_url.trim().is_empty()
        || state.stripe.studio_cancel_url.trim().is_empty()
    {
        return Err(billing_error(
            StatusCode::PRECONDITION_FAILED,
    ...
    let success_url = state.stripe.studio_success_url.trim().to_string();
    let cancel_url = state.stripe.studio_cancel_url.trim().to_string();
...
pub async fn create_agency_subscription_checkout(
    ...
    if state.stripe.checkout_success_url.trim().is_empty()
        || state.stripe.checkout_cancel_url.trim().is_empty()
    {
        return Err(billing_error(
            StatusCode::PRECONDITION_FAILED,
    ...
    let billing_ctx = get_or_create_agency_billing_context(&state, &agency_id).await?;
    let access = crate::billing::entitlements::get_agency_access_state(&state, &agency_id).await?;
    ...
    let mut cs_params = stripe_sdk::CreateCheckoutSession::new();
    let success_url = agency_checkout_success_url(state.stripe.checkout_success_url.as_str());
    cs_params.success_url = Some(success_url.as_str());
    cs_params.cancel_url = Some(state.stripe.checkout_cancel_url.as_str());
...
pub async fn change_agency_subscription_plan(
    ...
    let sub = crate::billing::payouts::fetch_subscription(&state, &subscription_id)
        .await
    ...
    crate::billing::payouts::sync_agency_subscription_from_stripe(
        &state,
        &agency_id,
        &subscription_id,
...
pub async fn create_agency_irl_booking_addon_checkout(
    ...
    if state.stripe.agency_irl_booking_price_id.trim().is_empty() {
        return Err(billing_error(
            StatusCode::PRECONDITION_FAILED,
            "stripe_price_not_configured",
...
pub async fn create_agency_studio_addon_checkout(
    ...
    if state.stripe.brand_studio_addon_price_id.trim().is_empty() {
        return Err(billing_error(
            StatusCode::PRECONDITION_FAILED,
            "stripe_price_not_configured",
    ...
    cs_params.client_reference_id = Some(agency_id.as_str());
    cs_params.line_items = Some(vec![stripe_sdk::CreateCheckoutSessionLineItems {
        price: Some(state.stripe.brand_studio_addon_price_id.clone()),
        quantity: Some(1),
        ..Default::default()
    }]);
...
pub async fn create_creator_subscription_checkout(
            ...
            Ok(subscription) => {
                ...
                if live_subscription {
                        ...
                        && subscription.status == stripe_sdk::SubscriptionStatus::Trialing
                    {
                        ...
                        portal_params.return_url = Some(state.stripe.creator_success_url.as_str());
    ...
    let mut cs_params = stripe_sdk::CreateCheckoutSession::new();
    cs_params.success_url = Some(state.stripe.creator_success_url.as_str());
    cs_params.cancel_url = Some(state.stripe.creator_cancel_url.as_str());
    cs_params.mode = Some(stripe_sdk::CheckoutSessionMode::Subscription);
...
pub async fn upgrade_creator_subscription(
    ...
    let res = req_client
        ...
        .basic_auth(&state.stripe.secret_key, Some(""))
        .form(&[
...
pub async fn create_brand_subscription_checkout(
    ...
    if has_default_payment_method {
        return create_brand_subscription_direct(
            &state,
            &user.id,
            &customer_id,
            &base_price_id,
...
async fn create_brand_subscription_direct(
    state: &AppState,
    brand_id: &str,
    customer_id: &str,
    price_id: &str,
    ...
    let mut sub_md = std::collections::HashMap::new();
    sub_md.insert("brand_id".to_string(), brand_id.to_string());
    sub_md.insert("billing_domain".to_string(), "brand".to_string());
    sub_md.insert("billing_target".to_string(), "base".to_string());
    sub_md.insert("plan".to_string(), target_plan.to_string());
    sub_md.insert("billing_cycle".to_string(), billing_cycle.to_string());

    let mut params = stripe_sdk::CreateSubscription::new(customer_id_parsed.clone());
    params.items = Some(vec![stripe_sdk::CreateSubscriptionItems {
        price: Some(price_id.to_string()),
        ..Default::default()
    }]);
    params.metadata = Some(sub_md);
    params.trial_period_days = if should_start_trial {
        Some(state.brand_trial_days)
    } else {
        None
    };
    ...
    let subscription = stripe_sdk::Subscription::create(&client, params)
        .await
        .map_err(|e| {
            warn!(error = %e, brand_id = %brand_id, "failed to create subscription directly");
            (StatusCode::BAD_GATEWAY, format!("stripe_error: {}", e))
        })?;
    ...
    info!(
        ...
        trial_applied = should_start_trial,
        "created brand subscription directly with default payment method"
    );

    let frontend_url = state.frontend_url.trim();
    let mut success_url = Url::parse(frontend_url).map_err(|e| {
        (
            StatusCode::PRECONDITION_FAILED,
            format!("invalid_frontend_url:{}", e),
        )
    })?;
    success_url.set_path("/BrandDashboard");
    {
        let mut query = success_url.query_pairs_mut();
        query.append_pair("billing_success", "1");
        query.append_pair("plan", target_plan);
    }

    Ok(Json(AgencyCheckoutResponse {
        checkout_url: success_url.to_string(),
        seats_limit: None,
        invoice_id: None,
        invoice_status: None,
        invoice_url: None,
...
pub async fn create_campaign_offer_checkout(
    ...
    if !crate::campaigns::offer_status_is_signed(&json!(offer)) {
        return Err(billing_error(
            StatusCode::BAD_REQUEST,
    ...
    if target_type == "agency" && billing_request_id.is_empty() {
        match crate::campaigns::ensure_campaign_billing_stub(&state, &offer_id).await {
            Ok(stub_id) => {
...
pub async fn get_agency_billing_status(
    ...
    let effective_plan_tier = match access.effective_tier {
        crate::billing::entitlements::PlanTier::Enterprise => "enterprise",
        crate::billing::entitlements::PlanTier::Pro => "pro",
        crate::billing::entitlements::PlanTier::Basic => "basic",
        crate::billing::entitlements::PlanTier::Free => "none",
    }
    .to_string();
...
pub async fn get_brand_billing_status(
    ...
    // Brand trials are not currently tracked in the database
    let trial_active = false;
    let trial_ends_at_str: Option<String> = None;
...
pub async fn check_license_expiration_alerts_cron(
    ...
    match crate::licensing::requests::check_license_expiration_alerts_all_brands(&state).await {
...
pub async fn auto_archive_expired_licensing_requests_cron(
    ...
    match crate::licensing::requests::auto_archive_expired_licensing_requests(&state).await {
...
pub async fn get_stripe_publishable_key(State(state): State<AppState>) -> Json<serde_json::Value> {
    Json(json!({ "publishable_key": state.stripe.publishable_key }))
}
...
pub async fn create_brand_payment_method_setup_intent(
    ...
    params.metadata = Some(
        vec![("billing_domain".to_string(), "brand".to_string())]
            .into_iter()
            .collect(),
    );
...
pub async fn get_brand_payment_methods(
    ...
    Ok(Json(GetPaymentMethodsResponse {
        payment_methods,
...
pub async fn set_brand_primary_payment_method(
    ...
    // Try to parse local payment method
    let local_payment_method: Option<PaymentMethodInfo> = if let Ok(resp) = payment_method_result {
    ...
    let payment_method: PaymentMethodInfo = if let Some(pm) = local_payment_method {
        pm
    } else {
    ...
    // Update Stripe customer's default payment method if customer exists
    ...
    // Update brand with new primary payment method
    state
        .pg
        .from("brands")
        .eq("id", &user_id)
