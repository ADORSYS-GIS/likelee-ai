...
pub async fn create_onboarding_link(
    ...
    link_params.return_url = Some(state.stripe.return_url.as_str());
    link_params.refresh_url = Some(state.stripe.refresh_url.as_str());
        ...
        Err(e) => {
            ...
            if msg.contains("relation") && msg.contains("does not exist") {
                ...
                return (
                    StatusCode::OK,
                    Json(
                        json!({"balances": [], "allowed_currencies": state.payout.allowed_currencies}),
                    ),
                );
            }
            ...
            && (text.contains("does not exist") || text.contains("column"))
        {
            ...
            if !stripe_account_id.is_empty() {
                ...
                stripe_balances = fetch_connected_balance_rows(
                    &client,
                    &stripe_account_id,
                    &state.payout.allowed_currencies,
                )
                .await;
            }
            ...
            return (
                ...
                Json(json!({
                    ...
                    "allowed_currencies": state.payout.allowed_currencies
                })),
            );
        }
        return sanitized_error_response(status.as_u16(), text);
...
pub async fn request_payout(
    ...
    let currency = payload
        .currency
        .unwrap_or_else(|| state.payout.currency.clone())
        .to_uppercase();
        ...
        .any(|c| c == &currency)
    {
        return (
            StatusCode::BAD_REQUEST,
            Json(
                json!({"status":"error","error":"unsupported_currency","allowed": state.payout.allowed_currencies}),
            ),
        );
    }

    // Likelee payouts are instant-only.
    if !state.payout.instant_enabled {
        return (
            StatusCode::BAD_REQUEST,
    ...
    let stripe_available_cents =
        fetch_connected_available_cents(state.stripe.secret_key.as_str(), &account_id, &currency)
            .await
            .unwrap_or(0);
...
pub async fn stripe_webhook(
    ...
    if let Err(e) = verify_stripe_signature(&payload, &sig, &state.stripe.webhook_secret) {
        return (
            StatusCode::BAD_REQUEST,
...
fn stripe_subscription_to_plan_tier_from_price_id(
    ...
    if (!state.stripe.creator_pro_price_id.trim().is_empty()
        && price_id == state.stripe.creator_pro_price_id)
        || (!state.stripe.creator_pro_annual_price_id.trim().is_empty()
            && price_id == state.stripe.creator_pro_annual_price_id)
    {
        return Some("pro");
    }
    if (!state.stripe.creator_basic_price_id.trim().is_empty()
        && price_id == state.stripe.creator_basic_price_id)
        || (!state.stripe.creator_basic_annual_price_id.trim().is_empty()
            && price_id == state.stripe.creator_basic_annual_price_id)
    {
        return Some("basic");
    }

    // Legacy: base-plan pricing
    if !state.stripe.agency_pro_base_price_id.trim().is_empty()
        && price_id == state.stripe.agency_pro_base_price_id
    {
        return Some("pro");
    }
    if !state.stripe.agency_basic_base_price_id.trim().is_empty()
        && price_id == state.stripe.agency_basic_base_price_id
    {
        return Some("basic");
    }

    // Backward compatibility
    if !state.stripe.scale_price_id.trim().is_empty() && price_id == state.stripe.scale_price_id {
        return Some("pro");
    }
    if !state.stripe.agency_price_id.trim().is_empty() && price_id == state.stripe.agency_price_id {
        return Some("basic");
    }
    None
}
...
fn stripe_subscription_to_interval_from_price_id(state: &AppState, price_id: &str) -> &'static str {
    if (!state.stripe.creator_pro_annual_price_id.trim().is_empty()
        && price_id == state.stripe.creator_pro_annual_price_id)
        || (!state.stripe.creator_basic_annual_price_id.trim().is_empty()
            && price_id == state.stripe.creator_basic_annual_price_id)
    {
...
fn stripe_subscription_has_irl_booking_addon(
    ...
    let irl_price_id = state.stripe.agency_irl_booking_price_id.trim();
...
fn stripe_subscription_plan_interval(
    ...
    for item in sub.items.data.iter() {
        ...
        if price_id == state.stripe.agency_basic_base_annual_price_id
            || price_id == state.stripe.agency_basic_headcount_annual_price_id
            || price_id == state.stripe.agency_pro_base_annual_price_id
            || price_id == state.stripe.agency_pro_headcount_annual_price_id
            || price_id == state.stripe.agency_irl_booking_annual_price_id
        {
            return "year";
        }
    }
...
fn brand_subscription_to_plan_tier_from_price_id(
    ...
    if (!state.stripe.brand_pro_price_id.trim().is_empty()
        && price_id == state.stripe.brand_pro_price_id)
        || (!state.stripe.brand_pro_annual_price_id.trim().is_empty()
            && price_id == state.stripe.brand_pro_annual_price_id)
    {
        return Some("pro");
    }
    if (!state.stripe.brand_basic_price_id.trim().is_empty()
        && price_id == state.stripe.brand_basic_price_id)
        || (!state.stripe.brand_basic_annual_price_id.trim().is_empty()
            && price_id == state.stripe.brand_basic_annual_price_id)
    {
        return Some("basic");
    }
    None
...
fn brand_subscription_has_studio_addon(state: &AppState, sub: &stripe_sdk::Subscription) -> bool {
    ...
    sub.items.data.iter().any(|item| {
        item.price
            .as_ref()
            .map(|price| price.id == state.stripe.brand_studio_addon_price_id)
            .unwrap_or(false)
    })
}
...
pub async fn request_agency_payout(
        ...
        .any(|c| c == &currency)
    {
        return (
            ...
            Json(json!({
                ...
                "allowed": state.payout.allowed_currencies
            })),
        );
    }

    // Check minimum payout amount
    if (payload.amount_cents as u32) < state.payout.min_amount_cents {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({
                "status":"error",
                "error":"below_minimum",
                "min_cents": state.payout.min_amount_cents
            })),
        );
    }
    ...
    // Compute fee
    let fee_cents = (payload.amount_cents * (state.payout.fee_bps as i64) + 9999) / 10000;
    let net_cents = payload.amount_cents - fee_cents;

    // Payouts are executed on the CONNECTED account balance.
    // Therefore, the cashout ceiling must be based on Stripe (not internal ledger balances).
    let stripe_available_cents = fetch_connected_available_cents(
        state.stripe.secret_key.as_str(),
        &stripe_account_id,
        &currency,
    )
    .await
    ...
    let status = if (payload.amount_cents as u32) <= state.payout.auto_approve_threshold_cents {
        "approved"
    } else {
        "pending"
    };
