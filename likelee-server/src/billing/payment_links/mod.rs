...
use crate::billing::entitlements::{get_agency_plan_tier, PlanTier};
...
pub async fn request_creator_payout(
    ...
    if payload.amount_cents < state.payout.min_amount_cents as i64 {
        return Err((
            StatusCode::BAD_REQUEST,
            format!(
                "Minimum payout amount is {} cents",
                state.payout.min_amount_cents
            ),
        ));
    }
    ...
    let auto_approve = payload.amount_cents <= state.payout.auto_approve_threshold_cents as i64;
