...
pub async fn create_campaign_offer_checkout(
    ...
    if !crate::brands::campaigns::offer_status_is_signed(&json!(offer)) {
        return Err(billing_error(
            StatusCode::BAD_REQUEST,
    ...
    if target_type == "agency" && billing_request_id.is_empty() {
        match crate::brands::campaigns::ensure_campaign_billing_stub(&state, &offer_id).await {
            Ok(stub_id) => {
...
pub async fn check_license_expiration_alerts_cron(
    ...
    match crate::licenses::requests::check_license_expiration_alerts_all_brands(&state).await {
