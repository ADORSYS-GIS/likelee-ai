            ...
            .await
        {
            Ok(mut res) => {
                if res.status().is_success() {
                    ...
                    let status = v.get("decision").and_then(|d| d.get("status")).and_then(|s| s.as_str()).unwrap_or("pending").to_lowercase();
            ...
            Err(e) => { debug!(error = %e, "Veriff decision request error"); }
        }
    }
    Ok(Json(rows))
}
...
#[derive(Deserialize)]
struct VeriffWebhookDecision { status: String }
#[derive(Deserialize)]
struct VeriffWebhookSession { id: String }
#[derive(Deserialize)]
...
pub async fn veriff_webhook(State(state): State<AppState>, headers: axum::http::HeaderMap, body_bytes: axum::body::Bytes) -> Result<axum::http::StatusCode, (axum::http::StatusCode, String)> {
    let sig_hdr = headers.get("x-hmac-signature").or_else(|| headers.get("vrf-hmac-sigature"));
    let provided = sig_hdr.and_then(|v| v.to_str().ok()).ok_or((axum::http::StatusCode::UNAUTHORIZED, "missing signature".to_string()))?;
    let computed = compute_hmac_hex(&state.veriff.shared_secret, &body_bytes);
    if !constant_time_eq(&computed, provided) {
        warn!("Invalid webhook signature");
        return Err((axum::http::StatusCode::UNAUTHORIZED, "invalid signature".into()));
    }

    let parsed: VeriffWebhookBody = serde_json::from_slice(&body_bytes).map_err(|e| (axum::http::StatusCode::BAD_REQUEST, e.to_string()))?;
    let user_id = parsed.vendor_data.ok_or((axum::http::StatusCode::BAD_REQUEST, "missing vendorData".into()))?;
    ...
    update_profile(&state, &user_id, &payload).await.map_err(|e| (axum::http::StatusCode::INTERNAL_SERVER_ERROR, e))?;
