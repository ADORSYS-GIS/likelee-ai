...
pub async fn create_catalog(
    ...
    if !client_email.is_empty() {
        ...
        match crate::messaging::email::send_plain_email(&state, &client_email, &subject, &body) {
            Ok(_) => {
                email_sent = true;
                // Mark sent_at
                let _ = state
...
pub async fn get_public_catalog(
    ...
    if !agency_id.is_empty() {
        if let Ok(refs) =
            crate::agency::agency_talent_refs::list_agency_talent_refs(&state, agency_id, None).await
        {
            for r in refs {
                let name = r.full_name.trim().to_string();
                if name.is_empty() {
