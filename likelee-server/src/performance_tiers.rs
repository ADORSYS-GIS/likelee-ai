...
pub async fn get_performance_tiers(
    ...
    let (resp_tiers, resp_config, resp_talents, resp_stats) = tokio::try_join!(
        ...
        state.pg.from("agencies").select("performance_config").eq("id", agency_id).execute(),
        // 3. Fetch Talents
        state.pg.from("agency_users")
    ...
    if !resp_tiers.status().is_success() {
        return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Tiers Error: {}", resp_tiers.status())));
    }
    let text_tiers = resp_tiers.text().await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Read tiers: {}", e)))?;
    let tiers_db: Vec<TierRuleDb> = serde_json::from_str(&text_tiers).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Parse tiers: {}", e)))?;
    ...
    if !resp_config.status().is_success() {
        return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Config Error: {}", resp_config.status())));
    }
    let text_config = resp_config.text().await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Read config: {}", e)))?;
    let agency_data: Vec<serde_json::Value> = serde_json::from_str(&text_config).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Parse config: {}", e)))?;
    ...
    if !resp_talents.status().is_success() {
        return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Talents Error: {}", resp_talents.status())));
    }
    let text_talents = resp_talents.text().await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Read talents: {}", e)))?;
    let talents_json: Vec<serde_json::Value> = serde_json::from_str(&text_talents).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Parse talents: {}", e)))?;

    // 4. Process Stats
    if !resp_stats.status().is_success() {
        return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("Stats Error: {}", resp_stats.status())));
    }
    let text_stats = resp_stats.text().await.map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Read stats: {}", e)))?;
    let stats: Vec<PerformanceStats> = serde_json::from_str(&text_stats).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Parse stats: {}", e)))?;
    ...
    println!("Performance Tiers: DB parallel fetch took {:?}, Total took {:?}", db_time, total_time);
