...
pub async fn list_brands_storage(
    ...
    for brand in brands_array {
        ...
        let limit = crate::brands::storage::ensure_brand_storage_settings_row(&state, &brand_id)
            .await
            .unwrap_or(5_368_709_120);
        let used = crate::brands::storage::get_brand_used_storage_bytes(&state, &brand_id)
            .await
            .unwrap_or(0);
