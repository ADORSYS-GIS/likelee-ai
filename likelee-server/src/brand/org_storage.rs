...
pub async fn save_from_url(
    ...
    if org_type == "brand" {
        let limit =
            crate::brand::brand_storage::ensure_brand_storage_settings_row(&state, &org_id).await?;
        let used = crate::brand::brand_storage::get_brand_used_storage_bytes(&state, &org_id).await?;
        ...
        let limit = crate::agency::agencies::ensure_storage_settings_row(&state, &org_id).await?;
        let used = crate::agency::agencies::get_agency_used_storage_bytes(&state, &org_id).await?;
        if used + new_size > limit {
            return Err((
                StatusCode::PAYLOAD_TOO_LARGE,
                "storage_quota_exceeded".into(),
