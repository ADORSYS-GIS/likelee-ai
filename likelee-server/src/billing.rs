...
#[derive(Debug, Deserialize)]
pub struct CreatorCheckoutSessionSyncRequest {
    #[serde(default)]
    pub session_id: String,
}

#[derive(Debug, Serialize)]
pub struct CreatorCheckoutSessionSyncResponse {
...
async fn fetch_creator_checkout_sync_state(
    state: &AppState,
    creator_id: &str,
) -> Result<CreatorCheckoutSessionSyncResponse, (StatusCode, String)> {
    let creator_resp = state
        .pg
        .from("creators")
        .select("plan_tier,trial_started_at,trial_basic_started_at,trial_pro_started_at")
        .eq("id", creator_id)
        .limit(1)
        .execute()
        .await
    ...
    let rows: Vec<serde_json::Value> = serde_json::from_str(&creator_text).unwrap_or_default();
    let row = rows.first().cloned().unwrap_or_else(|| json!({}));
    ...
    let billed_tier = match plan_tier_str.as_str() {
        "basic" => crate::entitlements::PlanTier::Basic,
        "pro" => crate::entitlements::PlanTier::Pro,
    ...
    Ok(CreatorCheckoutSessionSyncResponse {
...
pub async fn sync_creator_checkout_session(
    State(state): State<AppState>,
    user: AuthUser,
    Json(payload): Json<CreatorCheckoutSessionSyncRequest>,
) -> Result<Json<CreatorCheckoutSessionSyncResponse>, (StatusCode, String)> {
    ...
    if session_id_raw.is_empty() {
        ...
        if subscription_id.is_empty() {
            return Ok(Json(
                fetch_creator_checkout_sync_state(&state, &creator_id).await?,
            ));
        }

        crate::payouts::sync_creator_subscription_from_stripe(
            &state,
            &creator_id,
            subscription_id.as_str(),
            customer_id.as_deref(),
    ...
    let session_creator_id = session
        .client_reference_id
        .as_deref()
        .or_else(|| {
            session
    ...
    if session_creator_id != creator_id {
        return Err(billing_error(
            StatusCode::FORBIDDEN,
            "session_mismatch",
    ...
    let expected_plan_tier = session
        .metadata
        .as_ref()
        .and_then(|m| m.get("plan_tier"))
        .map(|v| v.trim().to_string());

    let subscription_id = session
        .subscription
        .as_ref()
        .map(|sub| sub.id().as_str().to_string())
        .unwrap_or_default();
    ...
    let customer_id = session
        .customer
        .as_ref()
        .map(|cust| cust.id().as_str().to_string())
        .filter(|id| !id.is_empty());

    let mut latest_state = CreatorCheckoutSessionSyncResponse {
        entitlement_tier: "free".to_string(),
        plan_tier: "free".to_string(),
        trial_active: false,
    };
    ...
    for attempt in 0..4 {
        ...
        latest_state = fetch_creator_checkout_sync_state(&state, &creator_id).await?;

        let plan_ready = expected_plan_tier
            .as_deref()
            .map(|expected| {
                latest_state.entitlement_tier == expected || latest_state.plan_tier == expected
            })
            .unwrap_or(true);
        if plan_ready {
            break;
