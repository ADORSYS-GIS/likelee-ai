...
pub async fn get_calendly_booking_url(
    ...
    let booking_url = state.calendly.booking_url.trim();
...
pub async fn handle_calendly_webhook(
    ...
    let signing_key = state.calendly.webhook_signing_key.trim();
...
pub async fn schedule_calendly_invitee(
    ...
    let mut token = state.calendly.api_token.trim().to_string();
