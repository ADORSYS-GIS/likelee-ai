...
use crate::brands::campaigns::{resolve_agency_name, resolve_brand_name, resolve_creator_name};
...
pub async fn search_marketplace_profiles(
    ...
    if entity_type == "creator" {
        ...
        if user.role == "agency" {
            crate::agencies::marketplace_contracts::sync_open_contracts_for_agency(
                &state,
                &effective_agency_id,
            )
            .await?;
        ...
        for row in creators_rows {
            ...
            let marketplace_contract = contract_by_creator_id
                .get(creator_id)
                .map(crate::agencies::marketplace_contracts::parse_contract_summary)
...
pub async fn create_marketplace_connection_request(
    ...
    if profile_type == "creator" {
        ...
        let contract_response =
            crate::agencies::marketplace_contracts::create_marketplace_connect_contract(
                &state,
                &effective_agency_id,
                &creator_id,
                &payload,
