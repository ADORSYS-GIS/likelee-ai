...
pub async fn list_invites(
    ...
    crate::agencies::marketplace_contracts::sync_open_contracts_for_creator(&state, &creator_id)
        .await?;
    ...
    for invite in &mut invites {
        invite.marketplace_contract =
            crate::agencies::marketplace_contracts::get_latest_contract_for_pair(
                &state,
                &invite.agency_id,
                &invite.creator_id,
            )
...
pub async fn accept_invite(
    ...
    if let Some(contract_row) = contract_rows.first() {
        let synced =
            crate::agencies::marketplace_contracts::sync_contract_for_row(&state, contract_row)
                .await?;
...
pub async fn list_connections(
    ...
    for connection in &mut connections {
        connection.marketplace_contract =
            crate::agencies::marketplace_contracts::get_latest_live_contract_for_pair(
                &state,
...
pub async fn disconnect_agency(
        ...
        get_latest_contract_for_connection(&state, &agency_id, &creator_id).await?
    {
        ...
        if contract_status == "expired" || contract_status == "terminated" {
            crate::agencies::marketplace_contracts::remove_live_connection_for_contract_row(
                &state,
                &contract_row,
            )
            .await;
