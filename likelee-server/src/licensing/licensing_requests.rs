...
pub async fn notify_brand_license_expirations_lazy(
    ...
    for (id, end_date) in candidates {
        ...
        let _ = crate::messaging::notifications::send_brand_notification(
            state,
            brand_id,
            None,
            &subject,
                        ...
                        for r in arr {
                            ...
                            if let Some(email) = target_email {
                                if !sent_emails.contains(&email) {
                                    ...
                                    if let Err(e) = crate::messaging::email::send_plain_email(
                    ...
                    if let Ok(req_text) = req_resp.text().await {
                            ...
                            serde_json::from_str::<Vec<serde_json::Value>>(&req_text)
                        {
                            ...
                            for req in req_rows {
                                ...
                                // Log activity event
                                let _ = crate::admin::activity::log_activity_event(
                                    &state_clone,
                                    &brand_id,
                                    None,
                                    "agency",
...
pub async fn send_payment_link(
        ...
        .map(|s| s.to_string())
    {
        if !email.is_empty() {
            ...
            match crate::messaging::email::send_plain_email(&state, email, &subject, &body) {
                Ok(_) => {
                    email_sent = true;
