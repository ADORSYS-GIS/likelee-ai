...
-- Profiles payout integration columns
...
create index if not exists idx_payout_requests_profile on payout_requests(profile_id);
...
with
  credits as (
    select profile_id, currency, sum(amount_cents) as credit_cents
    from ledger_entries
    where entry_type = 'credit'
    group by profile_id, currency
  ),
  debits as (
    select profile_id, currency, sum(amount_cents) as debit_cents
    from ledger_entries
    where entry_type = 'debit'
    group by profile_id, currency
  ),
  holds as (
    select pr.profile_id, pr.currency, coalesce(sum(pr.amount_cents),0) as hold_cents
    from payout_requests pr
    where pr.status in ('approved','processing')
    group by pr.profile_id, pr.currency
  )
select
  coalesce(c.profile_id, d.profile_id, h.profile_id) as profile_id,
...
full outer join debits d
  on c.profile_id = d.profile_id and c.currency = d.currency
full outer join holds h
  on coalesce(c.profile_id, d.profile_id) = h.profile_id
