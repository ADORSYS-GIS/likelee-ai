...
set
  expires_at = coalesce(created_at, now()) + interval '7 days'
where
  expires_at is null;
...
-- Fix: relax the strict foreign key on talent_id. The UI passes IDs that might be
-- agency_users, creators, or agency_talent_relationships, so a strict FK here
-- causes DB inserts to fail for connected creators.
alter table public.agency_catalog_items drop constraint if exists agency_catalog_items_talent_id_fkey;

-- Fix: add missing INSERT/ALL policies for catalog child tables (items/assets/recordings
-- only had SELECT policies, so all agency creates were silently blocked by RLS)
...
add constraint organization_invites_role_check check (role in ('admin', 'project_manager', 'reviewer')) not valid;
...
create unique index if not exists idx_organization_invites_token_hash on public.organization_invites (token_hash)
where
...
create unique index if not exists uq_agency_talent_relationships_agency_talent on public.agency_talent_relationships (agency_id, talent_id);

drop index if exists public.uq_agency_talent_relationships_agency_creator;

create unique index if not exists uq_agency_talent_relationships_agency_creator on public.agency_talent_relationships (agency_id, creator_id);
...
create policy "Users can view their own agency profile" on public.agencies for
select
  using (
    auth.uid () = id
...
create policy "Agencies can view their own members" on public.agency_users for
select
  using (
    agency_id = auth.uid ()
    or (
...
create policy "Agencies can manage their own members" on public.agency_users for all using (
  agency_id = auth.uid ()
  or (
    public.is_agency_team_member (agency_id)
...
create policy "Agencies can view their agency talent connections" on public.agency_talent_relationships for
select
  using (
    agency_id = auth.uid ()
    or (
...
create policy "Agencies can create their agency talent connections" on public.agency_talent_relationships for insert
with
  check (
    agency_id = auth.uid ()
    or (
...
create policy "Authenticated users can upload public storage objects" on storage.objects for insert to authenticated
with
  check (bucket_id = 'likelee-public');
...
for update
  to authenticated using (bucket_id = 'likelee-public')
with
  check (bucket_id = 'likelee-public');

drop policy if exists "Authenticated users can delete public storage objects" on storage.objects;

create policy "Authenticated users can delete public storage objects" on storage.objects for delete to authenticated using (bucket_id = 'likelee-public');
...
create unique index if not exists idx_agency_talent_packages_access_token_019 on public.agency_talent_packages (access_token)
where
...
create or replace function public.fill_package_item_talent_id () returns trigger language plpgsql security definer
set
  search_path = public as $$
begin
...
create trigger fill_package_item_talent_id before insert
or
update on public.agency_talent_package_items for each row
...
set
  talent_id = it.talent_id::text
from
  public.agency_talent_package_items it
where
  i.package_id = it.package_id
  and i.creator_id is not null
...
set
  amount_cents = coalesce(amount_cents, total_amount_cents, 0)
where
...
update public.agency_payment_links apl
set
  agency_id = lr.agency_id
from public.licensing_requests lr
where apl.licensing_request_id = lr.id
  and lr.agency_id is not null
  and apl.agency_id is distinct from lr.agency_id;

with paid_signals as (
  select
    apl.id as payment_link_id,
    max(coalesce(lp.paid_at, p.paid_at, apl.paid_at, apl.updated_at, apl.created_at)) as resolved_paid_at,
    max(
      coalesce(
        nullif(lp.stripe_payment_intent_id, ''),
        nullif(p.stripe_payment_intent_id, ''),
        nullif(apl.stripe_payment_intent_id, '')
      )
    ) as resolved_payment_intent_id
  from public.agency_payment_links apl
  left join public.licensing_payouts lp
    on lp.payment_link_id = apl.id
    or (
      apl.licensing_request_id is not null
      and lp.licensing_request_id = apl.licensing_request_id
    )
  left join public.payments p
    on apl.licensing_request_id is not null
    and p.licensing_request_id = apl.licensing_request_id
  where (
      lp.status = 'paid'
      or p.status in ('paid', 'succeeded')
    )
  group by apl.id
)
update public.agency_payment_links apl
set
  status = 'paid',
  is_active = false,
  paid_at = coalesce(ps.resolved_paid_at, apl.paid_at, now()),
  stripe_payment_intent_id = coalesce(ps.resolved_payment_intent_id, apl.stripe_payment_intent_id),
  updated_at = now()
from paid_signals ps
where apl.id = ps.payment_link_id
  and (
    apl.status is distinct from 'paid'
    or apl.paid_at is null
    or apl.is_active is distinct from false
  );

update public.license_submissions ls
set
  status = 'archived',
  archived_at = coalesce(ls.archived_at, apl.paid_at, now())
from public.agency_payment_links apl
where apl.licensing_request_id = ls.licensing_request_id
  and apl.status = 'paid'
  and ls.status is distinct from 'archived';

update public.licensing_requests lr
set
  status = 'archived',
  archived_at = coalesce(lr.archived_at, apl.paid_at, now())
from public.agency_payment_links apl
where apl.licensing_request_id = lr.id
  and apl.status = 'paid'
  and lr.status is distinct from 'archived';
...
where apl.licensing_request_id = lr.id
  and apl.status = 'paid'
  and (
    lr.effective_end_date is null
    or lr.effective_end_date >= current_date
  );
...
alter table public.agency_payment_link_transfers
...
create or replace function public.sync_agency_user_creator_from_email () RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
set
  search_path = public as $$
BEGIN
...
create trigger sync_agency_user_creator_from_email BEFORE INSERT
or
update OF email,
creator_id on public.agency_users for EACH row
execute FUNCTION public.sync_agency_user_creator_from_email ();

create or replace function public.sync_creator_to_agency_users_by_email () RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER
set
  search_path = public as $$
BEGIN
...
after INSERT
or
update OF email on public.creators for EACH row
...
update public.agency_talent_relationships r
set
  creator_id = au.creator_id,
  updated_at = now()
from
  public.agency_users au
where
  r.creator_id is null
  and r.talent_id = au.id
  and au.creator_id is not null;

insert into
  public.agency_talent_relationships (agency_id, talent_id, creator_id, status)
select
  au.agency_id,
  au.id,
  au.creator_id,
...
from
  public.agency_users au
where
  au.creator_id is not null
  and au.role = 'talent'
  and not exists (
    select
      1
    from
      public.agency_talent_relationships r
    where
      r.agency_id = au.agency_id
      and r.talent_id = au.id
  )
  and not exists (
    select
...
drop constraint if exists scouting_prospects_status_check;

alter table public.scouting_prospects
add constraint scouting_prospects_status_check check (
  status in (
    'new',
    'new_lead',
    'contacted',
...
alter table public.scouting_events
drop constraint if exists scouting_events_status_check;

alter table public.scouting_events
add constraint scouting_events_status_check check (
  status in ('draft', 'planned', 'scheduled', 'published', 'completed', 'cancelled')
);
...
begin
  ...
  if exists (
    ...
    update public.bookings_campaigns bc
    set agency_id = b.agency_id
    from public.bookings b
    where bc.agency_id is null
      and bc.booking_id = b.id
      and b.agency_id is not null;
  end if;
end $$;

update public.bookings_campaigns
set status = coalesce(nullif(btrim(status), ''), 'created')
...
update public.creators real
set stripe_connect_account_id = orphan.stripe_connect_account_id,
    payouts_enabled = coalesce(orphan.payouts_enabled, real.payouts_enabled, false),
    last_payout_error = coalesce(nullif(orphan.last_payout_error, ''), real.last_payout_error),
    updated_at = now()
from public.agency_users au
join public.creators orphan
  on orphan.id = au.user_id
where real.id = au.creator_id
  and au.creator_id is not null
  and au.user_id is not null
  and au.creator_id <> au.user_id
  and orphan.stripe_connect_account_id is not null
  and btrim(orphan.stripe_connect_account_id) <> ''
  and (
    real.stripe_connect_account_id is null
    or btrim(real.stripe_connect_account_id) = ''
    or real.stripe_connect_account_id = orphan.stripe_connect_account_id
  );

notify pgrst,
'reload schema';

-- Fix: Update the check constraint on asset_type to allow 'image' (frontend default)
alter table public.agency_catalog_assets drop constraint if exists agency_catalog_assets_asset_type_check;
alter table public.agency_catalog_assets add constraint agency_catalog_assets_asset_type_check check (asset_type IN ('photo', 'video', 'digitals', 'comp_card', 'image'));
