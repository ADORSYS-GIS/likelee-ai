-- 2026-05-04_get_public_package_details_add_creator_id.sql
--
-- The interactions sub-query in get_public_package_details was missing
-- creator_id.  Independent connected creators have talent_id = null and
-- creator_id set, so the frontend could not seed initialSelections correctly
-- after a refetch, causing the Save Changes button to stay disabled when
-- trying to deselect an independent creator.
      ...
      'interactions', COALESCE((
        SELECT jsonb_agg(
          jsonb_build_object(
            ...
            'creator_id',  i.creator_id,   -- needed for independent connected creators
            'type',        i.type,
            'content',     i.content,
