...
export function useUnreadMessages(currentUserId?: string) {
  const { data: conversations = [] } = useQuery({
    ...
    queryFn: async () => {
      const data = await base44.get<{ conversations: Conversation[] }>(
        "/api/conversations",
      );
...
export function useChat(currentUserId?: string, userRole?: string) {
  ...
  const {
    ...
    queryFn: async () => {
      const data = await base44.get<{ contacts: Contact[] }>(
  ...
  const startConversation = useCallback(
    async (
      ...
      try {
        ...
        const data = await base44.post<{ conversation_id: string }>(
  ...
  const getParticipant = useCallback(
    (conversation: Conversation, perspectiveUserId: string): Participant => {
