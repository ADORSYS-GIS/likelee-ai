...
export const scoutingService = {
    async getUserAgencyId() {
        ...
        // 1. Check if user is an owner of an organization
        ...
        if (orgAsOwner) return orgAsOwner.id;

        // 2. Check if user is a member of an agency
        const { data: orgAsMember, error: memberError } = await supabase
            .from("agency_users")
            .select("agency_id")
            .eq("user_id", user.id)
            .maybeSingle();

        if (orgAsMember) return orgAsMember.agency_id;
