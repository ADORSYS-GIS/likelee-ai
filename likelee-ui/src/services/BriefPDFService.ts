...
export async function generateBriefPDF(
  ...
  currentY -= 30;
  page.drawText(`${campaignName} | ${brandName}`, {
    x: margin,
    y: currentY,
    size: 12,
    font: font,
  ...
  const renderSection = async (
    title: string,
    fields: { label?: string; key: string; isList?: boolean }[],
  ) => {
  ...
  await renderSection("Key Messages", [{ key: "key_messages", isList: true }]);
  ...
  // Reference Images
  const refImages = Array.isArray(brief?.reference_images)
