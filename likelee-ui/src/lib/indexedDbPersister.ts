...
const pendingIndexedDbSaves: Map<string, { data: unknown; agencyId?: string; timestamp: number }> = new Map();
const pendingLocalStorageSaves: Map<string, { data: unknown; timestamp: number }> = new Map();
...
async function flushPendingSaves() {
  ...
  if (pendingLocalStorageSaves.size > 0) {
    ...
    for (const [keyStr, value] of saves) {
      try {
        localStorage.setItem(`likelee_query_${keyStr}`, JSON.stringify({
          data: value.data,
  ...
  const unsubscribe = queryClient.getQueryCache().subscribe((event) => {
    if (event.type === 'updated' || event.type === 'added') {
      ...
      if (shouldPersistToIndexedDb(queryKey)) {
        ...
        const agencyId = typeof queryKey[1] === 'string' ? queryKey[1] : undefined;
...
async function loadPersistedCache(queryClient: QueryClient): Promise<void> {
  ...
  try {
    ...
    console.warn('[LocalStoragePersister] Failed to load from localStorage:', error);
  }
}
