...
export function CatalogBuilderWizard({
    ...
    const handleUploadAsset = async (talentId: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleUploadRecording = async (talentId: string, event: React.ChangeEvent<HTMLInputElement>) => {
        const files = Array.from(event.target.files || []);
        if (files.length === 0) return;
    ...
    const toggleAsset = (
        ...
        setForm((prev) => {
            const item = prev.items[talentId] ?? {
                talent_id: talentId,
                talent_name:
                    selectedTalents.find((t) => t.id === talentId)?.name ?? "",
                asset_ids: [],
                recording_ids: [],
            };
        ...
        setForm((prev) => {
            ...
            const updated: CatalogItem = {
                ...
                recording_ids: exists
                    ...
                    : [...item.recording_ids, { recording_id: recordingId, emotion_tag: emotionTag }],
            };
            return { ...prev, items: { ...prev.items, [talentId]: updated } };
        });
    };
    ...
    const selectRequest = (req: any) => {
        ...
        setForm((prev) => ({
            ...prev,
            title: prev.title || req.campaign_title || `Catalog – ${req.client_name || ""}`,
                ...
                <div className="flex items-center gap-1 px-6 pt-4">
                    {STEPS.map((s, i) => (
                        <React.Fragment key={s.id}>
                            <div
                                className={`flex items-center gap-1.5 text-xs font-semibold ${s.id === step
                                    ? "text-indigo-700"
                                    : i < stepIndex
                                        ? "text-green-600"
                                        : "text-gray-400"
                            ...
                            >
                                <div
                                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${s.id === step
                            ...
                            {loadingAssets ? (
                                ...
                                selectedTalents.map((talent) => {
                                    ...
                                    const selectedAssets =
                                        form.items[talent.id]?.asset_ids ?? [];
                                    return (
                                        <div key={talent.id}>
                                                ...
                                                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 ml-6">
                                                    ...
                                                    {assets.map((asset: any) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                {asset.asset_type === "video" || asset.type === "video" || asset.mime_type?.startsWith("video/") ? (
                                            ...
                                            {recordings.length === 0 ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <label className="cursor-pointer">
                                                        <input
                                                            ...
                                                            accept="audio/*"
                                                            className="hidden"
                                                            onChange={(e) => handleUploadRecording(talent.id, e)}
                                                            disabled={uploadingFor === talent.id}
                                                        />
                                                        <div className="flex items-center gap-1.5 text-xs font-semibold text-indigo-600 hover:text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors">
                                                            {uploadingFor === talent.id ? (
                                                ...
                                                <div className="space-y-2 ml-6">
                                                    <label className="cursor-pointer border-2 border-dashed border-gray-200 hover:border-indigo-500 hover:bg-indigo-50 transition-colors rounded-xl p-3 flex items-center gap-3 text-gray-500 hover:text-indigo-600 group w-full">
                                                        ...
                                                        <span className="text-sm font-semibold">Upload New Recording</span>
                                                    </label>
                                                    {recordings.map((rec: any) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div
                                                                    className={`w-8 h-8 rounded-full flex items-center justify-center ${isSelected
                                                                        ? "bg-indigo-600"
                                                                        : "bg-gray-100"
                                                                        }`}
                                                                >
