...
export const DocuSealBuilderModal: React.FC<DocuSealBuilderModalProps> = ({
    ...
    const [docusealUserEmail, setDocusealUserEmail] = useState<string | null>(null);
        ...
        if (open) {
            ...
            createBuilderToken(name, docusealTemplateId, externalId)
                ...
                .catch((err) => {
                    toast({
                        title: "Error",
                        description: "Failed to initialize DocuSeal builder: " + err.message,
                        variant: "destructive",
                    });
                    onClose();
                })
        ...
        <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
            <DialogContent className="fixed !inset-0 !z-[9999] bg-background w-screen h-screen !max-w-none !translate-x-0 !translate-y-0 !rounded-none border-none !pt-20 p-0 flex flex-col outline-none">
                ...
                <DialogDescription className="sr-only">
                    Design your contract template by uploading a PDF and adding signature fields
                </DialogDescription>
                            ...
                            <DocusealBuilder
                                ...
                                onSave={(data: any) => {
                                    ...
                                    if (data?.id && hasFields) {
                                        ...
                                        toast({
                                            title: "Layout Saved",
                                            description: "The document layout has been updated and sent.",
                                        });
                                    } else if (data?.id && !hasFields) {
                                        console.log("ignoring save with no fields (likely just upload)");
                                    }
                                }}
