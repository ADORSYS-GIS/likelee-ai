          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex items-end gap-2.5 ${isSelf ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div className={`max-w-[80%] group relative flex flex-col ${isSelf ? "items-end" : "items-start"}`}>
                  <div
                  ...
                  >
                    {msg.is_deleted ? (
                      ...
                      <>
                        {msg.content}
                        {msg.edited_at && (
                          <span className="text-[9px] opacity-70 ml-1.5 align-middle">(edited)</span>
                        )}
                      </>
                    )}
                    ...
                    {isSelf && !msg.is_deleted && !isOptimistic && (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="start" className="w-24">
                          <DropdownMenuItem onClick={() => handleEditClick(msg.id, msg.content)}>Edit</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleDeleteClick(msg.id)} className="text-red-600">Delete</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    )}
                  </div>
                  ...
                  <div className={`flex items-center gap-1.5 mt-1 px-1`}>
                    ...
                    {isSelf && !msg.is_deleted && (
                      <span className={`text-[12px] leading-none ${msg.is_read ? "text-green-500 font-bold" : "text-gray-300"}`}>
                        {msg.is_read ? "✓✓" : "✓"}
                      </span>
                    )}
                  </div>
