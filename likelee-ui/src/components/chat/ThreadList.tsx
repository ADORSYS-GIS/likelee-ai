...
export function ThreadList({
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    const matchesFilter = filter === "all" || (conv.unread_count && conv.unread_count > 0);
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              filter === "all" ? "bg-indigo-600 text-white shadow-md shadow-indigo-100" : "bg-gray-50 text-gray-500 hover:bg-gray-100"
            }`}
          >
            All
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${
              filter === "unread" ? "bg-indigo-600 text-white shadow-md shadow-indigo-100" : "bg-gray-50 text-gray-500 hover:bg-gray-100"
            }`}
          >
            Unread
            {conversations.some(c => (c.unread_count || 0) > 0) && (
              <span className={`w-2 h-2 rounded-full ${filter === "unread" ? "bg-white" : "bg-indigo-500"}`} />
            )}
          </button>
        </div>
      </div>
          ...
          return (
            <li
              ...
              className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-all hover:bg-indigo-50/50 ${
                isActive ? "bg-indigo-50/80 border-l-4 border-indigo-500" : "border-l-4 border-transparent"
              }`}
            >
