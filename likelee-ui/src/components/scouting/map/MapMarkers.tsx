...
const createCustomIcon = (
  color: string,
  iconType: "prospect" | "event" | "signed" | "trip" | "prospect-only",
) => {
  let iconSvg = "";
  ...
  if (iconType === "prospect") {
    ...
    iconSvg =
      '<path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>';
  } else if (iconType === "signed") {
    // User icon - Bolder
    iconSvg =
      '<path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="7" r="4" stroke="white" stroke-width="3"/>';
  } else if (iconType === "event") {
    // Calendar icon - Bolder
    iconSvg =
      '<rect x="3" y="4" width="18" height="18" rx="2" ry="2" stroke="white" stroke-width="3"/><line x1="16" y1="2" x2="16" y2="6" stroke="white" stroke-width="3"/><line x1="8" y1="2" x2="8" y2="6" stroke="white" stroke-width="3"/><line x1="3" y1="10" x2="21" y2="10" stroke="white" stroke-width="3"/>';
  } else if (iconType === "trip") {
    // Plane icon - Bolder and more defined
    iconSvg =
      '<path d="M17.8 19.2L16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 2H4l-1 1 3 2 2 3 1-1v-3l2-2 3.5 5.3c.3.4.8.5 1.3.3l.5-.2c.4-.3.6-.7.5-1.2z" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" fill="white" fill-opacity="0.3"/>';
  } else if (iconType === "prospect-only") {
    // Clock icon - Bolder
    iconSvg =
      '<circle cx="12" cy="12" r="10" stroke="white" stroke-width="3"/><polyline points="12 6 12 12 16 14" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>';
  }
...
interface MapMarkersProps {
  ...
  signedProspects?: (ScoutingProspect & {
    coords: { lat: number; lng: number };
  })[];
  prospectsOnly?: (ScoutingProspect & {
    coords: { lat: number; lng: number };
  })[];
  events: (ScoutingEvent & { coords: { lat: number; lng: number } })[];
  trips?: any[];
  declinedProspects?: (ScoutingProspect & {
    coords: { lat: number; lng: number };
  })[];
  ...
  return (
    <>
      ...
      {prospectsOnly.map((prospect) => (
        ...
        >
          <Popup autoClose={false} closeOnClick={false}>
            <ProspectPopup prospect={prospect} onView={onViewProspect} />
          </Popup>
        </Marker>
      ))}
      ...
      {events.map((event) => (
        ...
        >
          <Popup autoClose={false} closeOnClick={false}>
            <EventPopup event={event} onEdit={onEditEvent} />
          </Popup>
        </Marker>
      ))}
