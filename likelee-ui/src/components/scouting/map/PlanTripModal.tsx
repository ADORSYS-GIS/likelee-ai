...
export const PlanTripModal = ({
  ...
  const [tripType, setTripType] = useState<
    "Open Scouting" | "Specific Casting" | "Event Coverage" | "Other"
  >(initialData?.trip_type || "Open Scouting");
  const [status, setStatus] = useState<"planned" | "ongoing" | "completed">(
    initialData?.status || "planned",
  );
  const [goal, setGoal] = useState(
    initialData?.prospects_added?.toString() || "10",
  );
  const [budget, setBudget] = useState(
    initialData?.total_cost?.toString() || "0",
  );
  const [approached, setApproached] = useState(
    initialData?.prospects_approached?.toString() || "0",
  );
  const [submitted, setSubmitted] = useState(
    initialData?.prospects_agreed?.toString() || "0",
  );
  const [added, setAdded] = useState(
    initialData?.prospects_added?.toString() || "0",
  );
  const [conversion, setConversion] = useState(
    initialData?.conversion_rate?.toString() || "0",
  );
              ...
              <div className="grid gap-3">
                ...
                <div className="flex flex-wrap gap-2 items-center p-4 bg-gray-50 rounded-[24px] border border-gray-200 min-h-[60px] shadow-inner">
                  {scouts.map((scout) => (
                    <div
                      key={scout}
                      className="flex items-center gap-2 px-4 py-2 bg-white text-indigo-800 rounded-xl text-xs font-black shadow-sm border border-indigo-100 animate-in fade-in slide-in-from-left-2"
                    >
                      {scout}
                      <button
                        onClick={() =>
                          setScouts(scouts.filter((s) => s !== scout))
                        }
              ...
              <div className="grid gap-3">
                ...
                <div className="grid grid-cols-4 gap-4">
                  {photos.map((photo, index) => (
                    <div
                      key={index}
                      className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-gray-200 group shadow-md"
                    >
                      <img
                        src={photo}
                        alt="Preview"
            ...
            onClick={async () => {
              try {
                ...
                toast({
                  ...
                  description:
                    "There was a problem saving your trip. Please try again.",
                  variant: "destructive",
                });
              } finally {
                setIsSaving(false);
