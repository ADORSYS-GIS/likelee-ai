...
import { ScoutingProspect, ScoutingEvent, ScoutingTrip } from "@/types/scouting";
import { Badge } from "@/components/ui/badge";
import { Calendar, MapPin, User, Instagram, Mail, Phone, Trash2, Target, DollarSign, Users } from "lucide-react";
...
export const TripPopup = ({
    ...
    return (
        <div className="p-2 min-w-[260px] font-sans">
            <div className="mb-4">
                <h4 className="font-black text-gray-900 text-base leading-tight mb-2">{trip.name}</h4>
                <Badge className={`${getStatusColor(trip.status)} border px-2 py-0.5 text-[10px] font-black uppercase tracking-wider`}>
                    {trip.status}
                </Badge>
            </div>
            ...
            <div className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-black text-gray-900">{formatDateRange(trip.start_date, trip.end_date)}</span>
                </div>

                <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500 font-bold">Target Goal:</span>
                    <span className="font-black text-gray-900">{trip.prospects_added || 0} prospects</span>
                </div>

                <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500 font-bold">Budget:</span>
                    <span className="font-black text-gray-900">${trip.total_cost?.toLocaleString() || "0"}</span>
                </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {(trip.scout_names && trip.scout_names.length > 0) ? (
                            trip.scout_names.map((scout, i) => (
                                <Badge key={i} variant="secondary" className="bg-indigo-50 text-indigo-700 border-indigo-100 font-black text-[10px] px-2 py-1 rounded-lg">
                                    {scout}
                                </Badge>
                            ))
                        ) : (
                            <span className="text-[10px] text-gray-400 font-medium italic">No scouts assigned</span>
                        )}
                    </div>
                </div>
            </div>
