...
export const CustomRatesModal: React.FC<CustomRatesModalProps> = ({ isOpen, onClose, creator, onSave }) => {
  ...
  const fetchRates = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE}/api/creator-rates?user_id=${creator.id}`);
      ...
      const combinedRates = allPossibleRates.map(p => {
        const existing = data.find((d: CustomRate) => d.rate_type === p.rate_type && d.rate_name === p.rate_name);
      ...
      setRates(getAllPossibleRates().map(p => ({ ...p, price_per_week_cents: 0 })));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getAllPossibleRates = () => {
    const contentRates = (creator.content_types || []).map((name: string) => ({ rate_type: 'content_type', rate_name: name, price_per_week_cents: 0 }));
    const industryRates = (creator.industries || []).map((name: string) => ({ rate_type: 'industry', rate_name: name, price_per_week_cents: 0 }));
        ...
        <div className="max-h-[60vh] overflow-y-auto p-1 pr-4">
          <div className="space-y-6">
            {rates.map((rate, index) => (
              <div key={`${rate.rate_type}-${rate.rate_name}`} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  <p className="font-medium text-gray-800">{rate.rate_name}</p>
                  <p className="text-xs text-gray-500 capitalize">{rate.rate_type.replace('_', ' ')}</p>
                </div>
            ...
            {!isLoading && rates.length === 0 && <p className="text-center text-gray-500 py-8">Select content types and industries in 'My Rules' to set custom rates.</p>}
          </div>
        </div>
        <DialogFooter>
