...
export default function BrandFileStorageView() {
  ...
  useEffect(() => {
    const previewable = files.filter(
      (f) => isPreviewableImage(f.mime_type) && f.public_url,
    );
  ...
  const handleRenameFolder = async () => {
    ...
    try {
      await updateBrandStorageFolder(renameFolderId, {
  ...
  const filteredFiles = useMemo(() => {
    ...
    if (sourceFilter !== "all") {
      result = result.filter(
        (f) => (f.source_type || "upload") === sourceFilter,
      );
    }
    return result;
  ...
  const usagePercent = usage
    ? Math.min(100, (usage.used_bytes / usage.limit_bytes) * 100)
    : 0;
  const usageColor =
    usagePercent > 90
      ? "bg-red-500"
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm text-gray-500">
              {bytesToHuman(usage.used_bytes)} /{" "}
              {bytesToHuman(usage.limit_bytes)}
            </span>
          </div>
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          >
            <TabsList className="bg-gray-100 rounded-xl p-1 h-9">
              <TabsTrigger
                value="all"
                className="rounded-lg px-3 text-sm data-[state=active]:bg-white data-[state=active]:shadow-sm"
              >
                All
              </TabsTrigger>
              <TabsTrigger
              ...
              >
                <div className="aspect-square bg-gray-50 flex items-center justify-center relative">
                  ...
                  <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="p-1.5 rounded-lg bg-white/90 shadow hover:bg-white">
                      ...
                      <DropdownMenuContent align="end">
                        {isPreviewableImage(file.mime_type) && (
                          <DropdownMenuItem
                            onClick={() => {
                              const url = thumbUrl || file.public_url;
                ...
                <div className="p-3">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {bytesToHuman(file.size_bytes)} ·{" "}
                    {isoToShortDate(file.created_at)}
                  </p>
                </div>
              </div>
            );
      ...
      <AlertDialog open={deleteFolderOpen} onOpenChange={setDeleteFolderOpen}>
        <AlertDialogContent className="rounded-3xl border-none shadow-2xl">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="h-11 px-6 rounded-xl font-bold">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleDeleteFolder}
              className="h-11 px-6 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl"
            >
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
