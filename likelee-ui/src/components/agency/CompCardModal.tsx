  ...
  const uploadCompCardPublic = async (format: "jpeg" | "pdf") => {
    ...
    let user = null as any;
    {
      ...
      if (!session) {
        ...
        toast({
          title: t("agencyDashboard.compCard.share.notSignedInTitle", {
            defaultValue: "Not signed in",
          }),
          description: t("agencyDashboard.compCard.share.signInAgain", {
            defaultValue:
      ...
      if (userErr || !u) {
        ...
        if (
          ...
          await supabase.auth.signOut();
          toast({
            title: t("agencyDashboard.compCard.share.sessionExpiredTitle", {
  ...
  const onShareSend = async () => {
    ...
    try {
      ...
      if (failed.length > 0 && sent === 0) {
        ...
        toast({
          title: t("agencyDashboard.compCard.share.partiallySent", {
            defaultValue: "Partially sent",
          }),
          description: t("agencyDashboard.compCard.share.partialDescription", {
        ...
        toast({
          title: t("agencyDashboard.compCard.share.sent", {
            defaultValue: "Sent",
          }),
      ...
      const msg = String(
        err?.message ||
          err ||
          t("agencyDashboard.compCard.share.failed", {
            defaultValue: "Failed to share comp card",
          }),
      );
      toast({
        title: t("agencyDashboard.compCard.share.shareFailed", {
          ...
          <div className="w-full lg:flex-1 bg-gray-50 p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-center overflow-y-auto">
            ...
            {previewTalentComputed ? (
              ...
              <div className="text-center text-gray-400">
                <p>
                  {t("agencyDashboard.compCard.talent.selectToPreview", {
                    defaultValue: "Select a talent to preview",
                  })}
                </p>
              </div>
            )}
