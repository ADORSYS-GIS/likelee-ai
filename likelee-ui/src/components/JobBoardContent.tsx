...
import {
  Search, Filter, MapPin, Clock, DollarSign, Briefcase, Building2,
  ExternalLink, Loader2, BookmarkCheck, Bookmark, ArrowLeft, AlertCircle
} from "lucide-react";
...
export default function JobBoardContent() {
  ...
  const categorizeJob = (job) => {
    ...
    const isCreativeAI = combined.includes('ai video') || combined.includes('ai film') || combined.includes('video') || combined.includes('creative') || combined.includes('ai content');
    ...
    if (combined.includes('freelance') || contractType.includes('freelance')) {
      jobType = 'Freelance';
    } else if (combined.includes('contract') || contractType.includes('contract')) {
      jobType = 'Contract';
    ...
    try {
      ...
      const allJobs = [...joobleJobs, ...adzunaJobs].filter(job => {
        ...
        if (title.includes('engineer') || title.includes('engineering') || 
            description.includes('engineer') || description.includes('engineering')) {
          return false;
        }
  ...
  const filterJobs = () => {
    let filtered = jobs.filter(job => {
      if (selectedCategories.length > 0) {
        const hasCategory = selectedCategories.some(cat => job.detected_categories.includes(cat));
    ...
    if (sortBy === "recent") {
      ...
      filtered = filtered.sort((a, b) => new Date(a.created) - new Date(b.created));
    ...
    <div className="space-y-6">
      ...
      <div className="border-b-2 border-gray-200 bg-white rounded-none">
        <div className="px-6 py-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Job Discovery Hub</h2>
          ...
          {showFilters && (
            <Card className="mt-4 p-6 border-2 border-gray-200 bg-gray-50 rounded-none">
              <div className="flex justify-between items-center mb-6">
                ...
                <Button onClick={clearFilters} variant="ghost" className="text-sm text-gray-600 hover:text-gray-900">
                  Clear all
                </Button>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div>
                  <Label className="text-sm font-medium text-gray-900 mb-3 block">Category / Role Type</Label>
                  <div className="space-y-2">
                    {categoryOptions.map((category) => (
                      <div key={category} className="flex items-center space-x-2">
                        ...
                        />
                        <label htmlFor={`category-${category}`} className="text-sm text-gray-700 cursor-pointer flex-1">
                          {category}
                        </label>
                      </div>
                    ))}
                  ...
                  <div className="space-y-2">
                    {jobTypeOptions.map((type) => (
                      <div key={type} className="flex items-center space-x-2">
                        ...
                        />
                        <label htmlFor={`jobtype-${type}`} className="text-sm text-gray-700 cursor-pointer flex-1">
                          {type}
                        </label>
                      </div>
                    ))}
                ...
                <div>
                  <Label className="text-sm font-medium text-gray-900 mb-3 block">Location Type</Label>
                  <div className="space-y-2">
                    {locationOptions.map((location) => (
                      <div key={location} className="flex items-center space-x-2">
                        ...
                        />
                        <label htmlFor={`location-${location}`} className="text-sm text-gray-700 cursor-pointer flex-1">
                          {location}
                        </label>
                      </div>
                    ))}
      ...
      <div className="px-6">
        {error && (
          <Card className="p-8 border-2 border-red-300 bg-red-50 mb-6 rounded-none">
            ...
            <Button onClick={fetchJobs} className="mt-4 rounded-none">Try Again</Button>
          </Card>
        )}
        ...
        {!loading && filteredJobs.length > 0 && (
          <div className="grid lg:grid-cols-[400px_1fr] gap-6 items-start">
            ...
            {showMobileDetails && selectedJob && (
              <div className="fixed inset-0 bg-white z-50 lg:hidden overflow-y-auto">
                <div className="sticky top-0 bg-white border-b-2 border-gray-200 p-4 flex items-center gap-4">
                  <Button onClick={() => setShowMobileDetails(false)} variant="ghost" className="p-2 rounded-none">
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {selectedJob.detected_categories.map((cat) => (
                      <Badge key={cat} className={`${getCategoryColor(cat)} text-sm px-3 py-1`}>{cat}</Badge>
                    ))}
                  </div>
                  ...
                  <div className="mb-8">
                    ...
                    {expandingJobId === selectedJob.id ? (
                      ...
                      <div className="text-gray-700 leading-relaxed prose max-w-none" dangerouslySetInnerHTML={{ __html: selectedJob.description }} />
                    )}
                  </div>

                  <div className="flex gap-3 sticky bottom-0 bg-white py-4 border-t-2 border-gray-200">
                    <Button onClick={() => window.open(selectedJob.redirect_url, '_blank')} className="flex-1 h-12 bg-[#F7B750] hover:bg-[#E6A640] text-white rounded-none">
                      Apply Now
                      <ExternalLink className="w-4 h-4 ml-2" />
                    </Button>
                    <Button onClick={() => handleSaveJob(selectedJob)} variant="outline" className="h-12 px-6 border-2 border-gray-300 rounded-none">
                      {savedJobs.has(selectedJob.id) ? <BookmarkCheck className="w-5 h-5 text-[#F7B750]" /> : <Bookmark className="w-5 h-5" />}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <div className={`space-y-3 lg:max-h-[calc(100vh-240px)] lg:overflow-y-auto lg:pr-2 ${showMobileDetails ? 'hidden' : 'block'} lg:block`}>
              {filteredJobs.map((job) => (
                <Card
                  key={job.id}
                  onClick={() => handleJobClick(job)}
                  className={`p-3 lg:p-5 border-2 cursor-pointer transition-all hover:shadow-md rounded-none ${
                    selectedJob?.id === job.id ? 'border-[#32C8D1] bg-blue-50' : 'border-gray-200 bg-white'
                  }`}
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm lg:text-base font-semibold text-gray-900 mb-1 line-clamp-2">{job.title}</h3>
                  ...
                  <div className="flex flex-wrap gap-1.5 lg:gap-2">
                    {job.detected_categories.slice(0, 2).map((cat) => (
                      <Badge key={cat} className={`${getCategoryColor(cat)} text-[10px] lg:text-xs px-1.5 lg:px-2 py-0.5`}>{cat}</Badge>
                    ))}
            ...
            <div className="hidden lg:block">
              <div ref={jobDetailsRef} className="sticky top-32" style={{ height: 'calc(100vh - 160px)' }}>
                {selectedJob ? (
                    ...
                    <div className="p-8">
                      <div className="flex items-start justify-between gap-4 mb-6">
                        ...
                        <div className="flex gap-2 flex-shrink-0">
                          <Button onClick={() => window.open(selectedJob.redirect_url, '_blank')} className="h-11 px-6 bg-[#F7B750] hover:bg-[#E6A640] text-white rounded-none">
                            Apply Now
                            <ExternalLink className="w-4 h-4 ml-2" />
                          </Button>
                          <button onClick={() => handleSaveJob(selectedJob)} className="p-3 hover:bg-gray-100 rounded-full transition-colors border-2 border-gray-300">
                      ...
                      <div className="flex flex-wrap gap-2 mb-6">
                        {selectedJob.detected_categories.map((cat) => (
                          <Badge key={cat} className={`${getCategoryColor(cat)} px-3 py-1`}>{cat}</Badge>
                        ))}
                      </div>
