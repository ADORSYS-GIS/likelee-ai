...
export default function Login() {
  ...
  const [userType, setUserType] = React.useState(() => {
    return localStorage.getItem("login_user_type") || "creator";
  });
  const [loginAttempted, setLoginAttempted] = React.useState(() => {
    return localStorage.getItem("login_attempted") === "true";
  });
  const updateLoginAttempted = (val: boolean) => {
    setLoginAttempted(val);
    if (val) {
  ...
  React.useEffect(() => {
    if (initialized && authenticated && profile) {
      ...
      // If we reached here, login was successful and role matches
      updateLoginAttempted(false);
  ...
  const handleUserTypeChange = (value: string) => {
    // If switching tabs while authenticated with a different role, logout immediately
    if (authenticated && profile && profile.role !== value) {
      logout();
    }
    ...
    setError(null);
    setLoading(false);
    localStorage.setItem("login_user_type", value);
  };
  ...
  // Show loading state when redirecting or when authenticated and not showing a mismatch error
  const isRoleMismatch = profile && profile.role !== userType;
  if (isRedirecting || (authenticated && !isRoleMismatch)) {
    return (
