...
export default function Login() {
  ...
  React.useEffect(() => {
    if (initialized && authenticated && profile) {
      // Enforce role-based login ONLY if a login was actually attempted
      // This prevents stale sessions from previous logins from triggering a logout
      // just because the user switched tabs on the login page.
      if (loginAttempted && profile.role !== userType) {
        const roleName =
          profile.role === "brand"
            ? "Brand"
            : profile.role === "agency"
        ...
        setError(`This account is not registered. Please Check tab `);
        logout();
