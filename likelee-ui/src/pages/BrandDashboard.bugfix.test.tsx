...
describe("Bug Condition 2: Team Management UI Missing Despite Backend Support", () => {
  ...
  it("should provide role management controls for authorized brand users", async () => {
    ...
    await waitFor(() => {
      ...
      const roleElements = screen.queryAllByText(
        /owner|admin|reviewer|project manager/i,
      );
    ...
    const results = await Promise.all(
      teamRoles.map(async (role) => {
              ...
              text: () =>
                Promise.resolve(
                  JSON.stringify({
                    ...
                    permissions:
                      role === "owner"
                        ? ["invite_team_members", "update_member_roles"]
                        : [],
                    members: [
                      {
                        user_id: "brand-user-1",
        ...
        try {
          await waitFor(
            () => {
              const teamManagementHeading =
                screen.queryByText(/team management/i);
