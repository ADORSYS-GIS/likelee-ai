...
import { Image as ImageIcon, Loader2, Download, Coins, ArrowLeft, Sparkles, Wand2 } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { createPageUrl } from "@/utils";

const imageModels = [
  { id: "fal-ai/flux/schnell", name: "Flux Schnell", cost: 2, description: "Ultra-fast generation" },
  { id: "fal-ai/flux/dev", name: "Flux Dev", cost: 3, description: "High quality, balanced speed" },
  { id: "fal-ai/flux-pro", name: "Flux Pro", cost: 5, description: "Professional grade" },
  { id: "fal-ai/flux-pro/v1.1", name: "Flux Pro v1.1", cost: 6, description: "Latest Flux model" },
  { id: "fal-ai/flux-realism", name: "Flux Realism", cost: 4, description: "Photorealistic images" },
  { id: "fal-ai/aura-flow", name: "Aura Flow", cost: 3, description: "Creative and artistic" },
  { id: "fal-ai/stable-diffusion-v3-medium", name: "Stable Diffusion v3", cost: 3, description: "Open source standard" },
  { id: "fal-ai/stable-diffusion-v35-large", name: "Stable Diffusion v3.5", cost: 4, description: "Latest SD version" },
  { id: "fal-ai/recraft-v3", name: "Recraft v3", cost: 4, description: "Design-focused model" },
  { id: "fal-ai/ideogram-v2", name: "Ideogram v2", cost: 5, description: "Text rendering expert" },
  { id: "fal-ai/ideogram-v2/turbo", name: "Ideogram v2 Turbo", cost: 3, description: "Fast text rendering" }
];
...
export default function StudioImage() {
  ...
  const { data: generations } = useQuery({
    ...
    queryFn: () => base44.entities.StudioGeneration.filter({ user_email: user.email, generation_type: 'image' }, '-created_date', 40),
    enabled: !!user
  });

  const generateMutation = useMutation({
    mutationFn: async (data) => {
      const { data: result } = await base44.functions.invoke('generateImage', data);
      return result;
    },
    onSuccess: async (result) => {
      ...
      await base44.entities.StudioGeneration.create({
        ...
        settings: { image_size: imageSize, num_images: numImages, guidance_scale: guidanceScale, num_steps: numSteps }
      });
    ...
    onError: (error) => {
      if (error.message?.includes('Insufficient credits') || error.response?.status === 402) {
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1 space-y-6">
            <Card className="p-6 bg-white/5 border border-white/10 rounded-lg">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-gray-300">Image Size</label>
                ...
                <Button
                  ...
                  disabled={!prompt || generateMutation.isPending || !!generatingJobId}
          ...
          <div className="lg:col-span-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {generations?.map((gen) => (
                <Card key={gen.id} className="p-2 bg-white/5 border border-white/10 rounded-lg hover:border-white/20 transition-all group">
                  {gen.status === 'completed' && gen.output_url ? (
                    <>
                      ...
                      <p className="text-xs text-gray-400 line-clamp-2 mb-1 px-1">{gen.prompt}</p>
              ...
              {!generations?.length && (
                <div className="col-span-full text-center py-16">
                  ...
                  <p className="text-gray-400 text-lg">No images yet. Create your first one!</p>
                </div>
              )}
            </div>
          </div>
          ...
          <Card className="max-w-md w-full p-8 bg-[#1A1A1F] border-2 border-white/20 rounded-xl">
            <div className="text-center">
              ...
              <p className="text-gray-400 mb-8">
                You need credits to generate images. Subscribe to one of our plans to start creating amazing AI content!
              </p>
