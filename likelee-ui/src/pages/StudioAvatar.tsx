...
export default function StudioAvatar() {
  const [prompt, setPrompt] = useState("A professional headshot with neutral expression");
  ...
  const { data: generations } = useQuery({
    ...
    queryFn: () => base44.entities.StudioGeneration.filter({ user_email: user.email, generation_type: 'avatar' }, '-created_date', 10),
    enabled: !!user
  });

  const generateMutation = useMutation({
    mutationFn: async (data) => {
      const { data: result } = await base44.functions.invoke('generateAvatar', data);
      return result;
    },
      ...
      <header className="px-6 py-6 border-b border-white/10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Badge className="bg-white/10 text-white">{credits?.plan_type || 'Free'}</Badge>
          </div>
        </div>
      </header>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            <Card className="p-6 bg-white/5 border border-white/10 rounded-lg mb-6">
              <div className="space-y-6">
                <div>
                  ...
                  {imageUrl && (
                    <img src={imageUrl} alt="Preview" className="mt-4 w-full max-w-xs rounded-lg" />
                  )}
                </div>

                <div>
                  <label className="block text-sm font-medium mb-2">Style Prompt (Optional)</label>
                  <Textarea
                    value={prompt}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {generations?.map((gen) => (
                <Card key={gen.id} className="p-3 bg-white/5 border border-white/10 rounded-lg">
              ...
              {!generations?.length && (
                <div className="col-span-2 text-center py-12">
                  <p className="text-gray-400">No avatars yet. Create your first one!</p>
                </div>
              )}
            </div>
          </div>
