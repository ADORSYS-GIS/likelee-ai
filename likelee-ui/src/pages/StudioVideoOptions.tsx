...
import { Film, ArrowLeft, Clock, Coins, Search, Image as ImageFrame } from "lucide-react";
...
export default function StudioVideoOptions() {
  ...
  const handleModelSelect = (model) => {
    navigate(createPageUrl("StudioVideo") + `?model=${encodeURIComponent(model.id)}&mode=${mode}`);
  };
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-white">{getModeTitle()}</h1>
                  <p className="text-sm text-gray-400">Choose a model to get started</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            {filteredModels.map((model) => (
              <Card
                ...
                className={`p-6 bg-white/5 border-2 ${
                  selectedModel?.id === model.id ? 'border-[#F18B6A]' : 'border-white/10'
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      ...
                      {model.badges.map((badge, idx) => (
                        <Badge 
                          key={idx}
                          className={
                            badge === "Hot" ? "bg-pink-500/20 text-pink-400 border-pink-500/30" :
                            badge === "New" ? "bg-green-500/20 text-green-400 border-green-500/30" :
                            badge === "Audio" ? "bg-purple-500/20 text-purple-400 border-purple-500/30" :
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10">
                        ...
                        <span className="text-sm text-gray-300">{model.duration}</span>
                      </div>
                      
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10">
                        <Coins className="w-3.5 h-3.5 text-yellow-400" />
                        <span className="text-sm text-gray-300">{model.credits} credits</span>
                      </div>

                      {model.badges.includes("End frame") && (
                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10">
                          <ImageFrame className="w-3.5 h-3.5 text-blue-400" />
                          <span className="text-sm text-gray-300">End frame</span>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {filteredModels.length === 0 && (
            <div className="text-center py-20">
              <p className="text-gray-400 text-lg">No models found matching "{searchQuery}"</p>
            </div>
          )}
        </div>
      </section>
