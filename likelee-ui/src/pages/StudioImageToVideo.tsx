...
import { Film, Upload, Loader2, Download, Coins, ArrowLeft, Play, Sparkles, ChevronDown, Clock, Search } from "lucide-react";
...
export default function StudioImageToVideo() {
  ...
  const generateMutation = useMutation({
    mutationFn: async (data) => {
      const { data: result } = await base44.functions.invoke('imageToVideo', data);
      return result;
    },
  ...
  const selectedModelData = imageToVideoModels.find(m => m.id === selectedModel);
        ...
        <div className="grid lg:grid-cols-[400px_1fr] gap-8">
          ...
          <div className="space-y-6">
            <Card className="p-6 bg-white/5 border border-white/10 rounded-xl space-y-6">
                      ...
                      {imageToVideoModels.map((model) => (
                        ...
                        >
                          <div className={`w-5 h-5 rounded-full border-2 flex-shrink-0 mt-1 ${
                            selectedModel === model.id ? 'border-[#F18B6A] bg-[#F18B6A]' : 'border-white/30'
                          }`}>
              ...
              <div>
                ...
                {imageUrl ? (
                  <div className="relative">
                    <img src={imageUrl} alt="Upload" className="w-full rounded-lg border border-white/10" />
                    <button
                      onClick={() => setImageUrl("")}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <div className={`w-4 h-4 bg-white rounded-full transition-transform ${translatePrompt ? 'translate-x-5' : 'translate-x-0.5'}`} />
                    </button>
                  </div>
                </div>
                <Textarea
              ...
              {/* Aspect Ratio */}
              <div>
                <label className="block text-sm font-medium mb-3 text-white">Aspect Ratio</label>
              ...
              {/* Output Number */}
              <div>
                <label className="block text-sm font-medium mb-3 text-white">Output Video Number</label>
              ...
              {showAdvanced && (
                <div className="space-y-4 pt-2">
                  <div>
                    <label className="block text-sm mb-2 text-gray-300">Motion Intensity</label>
                    <Slider defaultValue={[127]} max={255} step={1} className="w-full" />
                  </div>
                  <div>
                    <label className="block text-sm mb-2 text-gray-300">FPS</label>
                    <Slider defaultValue={[24]} min={6} max={30} step={6} className="w-full" />
                  </div>
                </div>
              )}
              ...
              <div className="pt-4 border-t border-white/10">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-lg font-bold text-white">14 Credits</span>
                </div>
                <Button
                  onClick={handleGenerate}
                  disabled={!imageUrl || generateMutation.isPending || !!generatingJobId}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {generations?.map((gen) => (
                <Card key={gen.id} className="p-3 bg-white/5 border border-white/10 rounded-lg hover:border-white/20 transition-all">
          ...
          <Card className="max-w-md w-full p-8 bg-[#1A1A1F] border-2 border-white/20 rounded-xl">
            <div className="text-center">
              ...
              <p className="text-gray-400 mb-8">
                You need credits to generate videos. Subscribe to one of our plans to start creating!
              </p>
