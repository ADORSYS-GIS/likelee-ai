...
import { CheckCircle2, Eye, Edit, Shield, Users, Mic, Image as ImageIcon, DollarSign, Globe, MapPin } from 'lucide-react'
...
export default function PublicProfile() {
  ...
  const [stats, setStats] = useState<any>({ active_campaigns: 0, completed_projects: 0, voice_profiles: 0 })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setProfile(p)
        setStats({
          active_campaigns: Array.isArray(json?.campaigns) ? json.campaigns.length : 0,
  ...
  const displayName = useMemo(() => profile.full_name || '', [profile.full_name])
  const location = useMemo(() => [profile.city, profile.state].filter(Boolean).join(', '), [profile.city, profile.state])
  const handleText = useMemo(() => (profile.platform_handle ? `@${profile.platform_handle}` : ''), [profile.platform_handle])
  const contentTypes: string[] = useMemo(() => Array.isArray(profile.content_types) ? profile.content_types : [], [profile.content_types])
  const industries: string[] = useMemo(() => Array.isArray(profile.industries) ? profile.industries : [], [profile.industries])
  const pricePerWeek = (typeof profile.base_monthly_price_cents === 'number')
    ...
    <div className="bg-gray-50 min-h-screen flex">
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center gap-3">
            <Avatar className="w-12 h-12 border-2 border-gray-200">
              {profile.headshot_url ? <AvatarImage src={profile.headshot_url} alt={displayName || 'User'} /> : null}
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <p className="font-bold text-gray-900 truncate max-w-[10rem]">{displayName || '—'}</p>
                {profile?.kyc_status === 'approved' && (
                  <Badge className="bg-gray-900 text-green-400 border border-green-500">Verified</Badge>
                )}
              </div>
              {location && <p className="text-xs text-gray-600 truncate max-w-[10rem]">{location}</p>}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Public Profile Preview</h1>
            <p className="text-gray-600">This is how brands see your profile</p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" className="border-2 border-gray-300" onClick={() => setShowCard(true)}>
              <Eye className="w-4 h-4 mr-2" />
              View Card
            </Button>
            <Button className="bg-gray-900 text-white hover:bg-gray-800" onClick={() => navigate('/CreatorDashboard?section=settings')}>
            ...
            <div className="flex items-start gap-4">
              <div className="-mt-12">
                <Avatar className="w-20 h-20 border-4 border-white">
                  {profile.headshot_url ? <AvatarImage src={profile.headshot_url} alt={displayName} /> : null}
                  <AvatarFallback className="bg-emerald-100 text-emerald-700 font-bold">{displayName[0]}</AvatarFallback>
                </Avatar>
              </div>
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600 mt-1 flex-wrap">
                  ...
                  {handleText && (<span className="flex items-center gap-1"><Globe className="w-4 h-4" /> {handleText}</span>)}
                </div>
                {profile.bio && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                  <Card className="p-4 border border-gray-200">
                    ...
                    <div className="text-3xl font-bold mt-1">{stats.active_campaigns}</div>
                  </Card>
                  <Card className="p-4 border border-gray-200">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="text-3xl font-bold mt-1">{stats.completed_projects}</div>
                  </Card>
                  <Card className="p-4 border border-gray-200">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="text-3xl font-bold mt-1">{stats.voice_profiles}</div>
                  </Card>
                </div>

                {contentTypes.length > 0 && (
                  <div className="mt-6">
                    <h3 className="font-semibold text-gray-900 mb-2">Open to Work With</h3>
                    <div className="flex flex-wrap gap-2">
                      {contentTypes.map((t) => (
                        <Badge key={t} className="bg-teal-100 text-teal-800">{t}</Badge>
                      ))}
                    </div>
                  </div>
                )}
                ...
                {typeof pricePerWeek === 'number' && (
                  <div className="mt-6">
                    <Card className="p-4 border-2 border-sky-200 bg-gradient-to-r from-sky-50 to-emerald-50">
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 className="font-semibold text-gray-900">Licensing Rate</h4>
                          <p className="text-sm text-gray-700">Base rate for cameo usage</p>
                          {openToNegotiations && (
                            <div className="mt-2 text-sm text-emerald-700 flex items-center gap-2">
                              <Shield className="w-4 h-4" /> Open to negotiations
                            </div>
                          )}
                        </div>
                        <div className="text-right">
                          <div className="text-3xl font-bold text-sky-600">${pricePerWeek}</div>
        ...
        <Alert className="mt-6 bg-blue-50 border border-blue-200">
          <AlertDescription className="text-blue-900 text-sm">
            This is a preview of how your profile appears to brands. Make sure your information is up-to-date to attract more opportunities.
          </AlertDescription>
        </Alert>
          ...
          <DialogContent className="sm:max-w-[380px] p-0 rounded-2xl overflow-hidden">
            ...
            <div className="bg-white p-5">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {(contentTypes.length ? contentTypes : industries).slice(0, 3).map((t: string) => (
                  <span key={t} className="px-3 py-1 rounded-full text-xs bg-gray-100 text-gray-700 border border-gray-200">
                    {t}
                  </span>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                {typeof pricePerWeek === 'number' && (
                  <div>
                    ...
                    <div className="text-lg font-bold text-gray-900">${Math.round((Number(profile.base_monthly_price_cents)||0)/100)}</div>
                  </div>
                )}
              ...
              <div className="flex items-center justify-between">
                <button className="flex-1 mr-2 border border-gray-300 rounded-full py-2 text-sm">Preview</button>
                <button className="flex-1 ml-2 border border-gray-300 rounded-full py-2 text-sm" onClick={() => navigate('/PublicProfile')}>Profile</button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
