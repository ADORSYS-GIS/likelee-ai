...
export default function TalentDashboard() {
  ...
  const { data: portfolio, isLoading: portfolioLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const profiles = await base44.entities.TalentPortfolio.filter({
  ...
  const { data: projects = [], isLoading: projectsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await base44.entities.TalentProject.filter(
        { user_email: currentUser.email },
        "-created_date",
      );
    },
  ...
  const featuredProject = projects.find(
    (p) => p.id === portfolio?.featured_project_id,
  );
  const conversionRate =
    portfolio?.inquiry_count > 0
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Tabs defaultValue="portfolio" className="space-y-6">
          <TabsList className="bg-white border-2 border-black p-1 rounded-none">
            <TabsTrigger value="portfolio" className="rounded-none">
              Portfolio
            </TabsTrigger>
            <TabsTrigger value="jobs" className="rounded-none">
              Job Feed
              {highMatches.length > 0 && (
                <Badge className="ml-2 bg-[#F18B6A] text-white text-xs">
                  {highMatches.length}
                </Badge>
              )}
            </TabsTrigger>
              ...
              {projects.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {projects.map((project) => (
                    <Card
                      key={project.id}
                      className="bg-white border-2 border-black rounded-none overflow-hidden group hover:shadow-xl transition-all"
                    >
                      {project.media_url && (
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                          ...
                          <div className="flex items-center gap-2">
                            {project.id !== portfolio?.featured_project_id && (
                              <Button
                                onClick={() =>
                                  setFeaturedMutation.mutate(project.id)
                                }
                                variant="ghost"
                                size="sm"
                                className="p-2"
            ...
            <div>
              ...
              {jobMatches.length === 0 ? (
                <Card className="p-12 text-center border-2 border-gray-200 rounded-none">
                  ...
                  <p className="text-gray-600 mb-6">
                    Complete your profile and upload projects to start receiving
                    matched opportunities
                  </p>
                  <Button
                    onClick={() => navigate(createPageUrl("AITalentBoard"))}
                ...
                <div className="space-y-4">
                  {jobMatches.map((match) => (
                    <Card
                      key={match.id}
                      className="p-6 bg-white border-2 border-black rounded-none hover:shadow-lg transition-all"
                    >
                      ...
                      <div className="flex gap-3">
                        <Button className="flex-1 bg-[#F18B6A] hover:bg-[#E07A5A] text-white border-2 border-black rounded-none">
                          Apply Now
                        </Button>
                        <Button
                          variant="outline"
              ...
              <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setProfileData({
                          ...profileData,
                          project_rate_min: Number(e.target.value),
                        })
                      }
                      className="border-2 border-gray-300 rounded-none"
                    />
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setProfileData({
                          ...profileData,
                          project_rate_max: Number(e.target.value),
                        })
                      }
                      className="border-2 border-gray-300 rounded-none"
                    />
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-600">
                        Show brands your current status
                      </p>
                    </div>
                    <select
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={platform}>
                        <Label
                          htmlFor={platform}
                          className="text-xs text-gray-600 mb-1 block capitalize"
                        >
                          {platform}
                        </Label>
                        <div className="flex items-center gap-2">
                ...
                <div className="mt-10 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      onClick={() =>
                        navigate(createPageUrl("LicensingSettings"))
                      }
                      variant="outline"
                      className="border-2 border-black rounded-none"
                    >
