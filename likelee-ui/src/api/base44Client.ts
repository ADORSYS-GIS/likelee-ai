...
function buildUrl(base: string, url: string, params?: RequestConfig["params"]): string {
...
let API_BASE = (import.meta as any)?.env?.VITE_API_BASE_URL as string | undefined;
if (!API_BASE) {
  const stored = typeof window !== "undefined" ? window.localStorage?.getItem("API_BASE_URL") || undefined : undefined;
  API_BASE = stored || "http://localhost:8787"; // sensible dev default for likelee-server
  // eslint-disable-next-line no-console
  console.warn(`[api] VITE_API_BASE_URL not set; using ${API_BASE}. Set VITE_API_BASE_URL to avoid proxying to the Vite dev server.`);
}
...
export const base44 = {
  ...
  async post<T = any>(url: string, data?: any, config?: RequestConfig): Promise<T> {
    ...
    const body = isForm ? data : data !== undefined ? JSON.stringify(data) : undefined;
