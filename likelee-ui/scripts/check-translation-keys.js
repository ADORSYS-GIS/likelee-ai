...
function analyzeTranslations() {
  ...
  for (const namespace of NAMESPACES) {
    ...
    for (const lang of LANGUAGES) {
      ...
      if (missingKeys.length > 0) {
        console.log(
          `  ✗ ${lang.toUpperCase()}: ${langKeys.length} keys (${missingKeys.length} missing)`,
        );
      } else {
        console.log(
          `  ✓ ${lang.toUpperCase()}: ${langKeys.length} keys (complete)`,
        );
      }
    }
    ...
    for (const lang of ["de", "es", "fr"]) {
      report.summary.byNamespace[namespace] +=
        report.namespaces[namespace].missing[lang]?.length || 0;
    }
  }
  ...
  for (const lang of ["de", "es", "fr"]) {
    ...
    console.log(
      `  • Missing keys for ${lang.toUpperCase()}: ${missingKeysPath}`,
    );
  }
